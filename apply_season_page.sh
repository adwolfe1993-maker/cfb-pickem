...
cat > "src/components/ui/tabs.tsx" << 'SCRIPT_EOF'
"use client"
...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "inline-flex flex-1 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-md px-3 py-1.5 text-sm font-medium transition-all outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm",
        className
      )}
      {...props}
    />
...
cat > "src/components/SeasonTabs.tsx" << 'SCRIPT_EOF'
'use client'
...
cat > "src/app/standings/page.tsx" << 'SCRIPT_EOF'
import { redirect } from 'next/navigation'
...
mkdir -p "src/app/standings/[seasonId]"
cat > "src/app/standings/[seasonId]/page.tsx" << 'SCRIPT_EOF'
import { redirect } from 'next/navigation'

export default function StandingsSeasonRedirect() {
  redirect('/season')
}
SCRIPT_EOF

mkdir -p "src/app/win-the-week"
cat > "src/app/win-the-week/page.tsx" << 'SCRIPT_EOF'
import { redirect } from 'next/navigation'

export default function WinTheWeekRedirect() {
  redirect('/season')
}
SCRIPT_EOF

mkdir -p "src/app/dn-history"
cat > "src/app/dn-history/page.tsx" << 'SCRIPT_EOF'
import { redirect } from 'next/navigation'

export default function DnHistoryRedirect() {
  redirect('/season')
}
SCRIPT_EOF

mkdir -p "src/app/similarities"
cat > "src/app/similarities/page.tsx" << 'SCRIPT_EOF'
import { redirect } from 'next/navigation'

export default function SimilaritiesRedirect() {
  redirect('/season')
}
SCRIPT_EOF

mkdir -p "src/components"
cat > "src/components/SiteNav.tsx" << 'SCRIPT_EOF'
